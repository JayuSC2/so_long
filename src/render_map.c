...
/*   Updated: 2024/04/06 16:52:21 by juitz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while (i < data->map->height)
    {
        ...
        while (j < data->map->width)
        {
            if (data->map->full[i][j] == '1')
                mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->wall, i * 16, j * 16);
            else if (data->map->full[i][j] == '0')
                mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->floor, i * 16, j * 16);
            else if (data->map->full[i][j] == 'C')
                mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->collectible, i * 16, j * 16);
            else if (data->map->full[i][j] == 'E')
                mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->exit_closed, i * 16, j * 16);
            else if (data->map->full[i][j] == 'P')
                mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->player_front, i * 16, j * 16);
            j++;
        }
    	i++;
    }
